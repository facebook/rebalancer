...
for ext in "$tmpdir/wheel/rebalancer/_rebalancer"*.so; do
    ...
    echo "repair_macos: added @loader_path/_lib to $(basename "$ext")"
done

# --- Patch 2: librebalancer.dylib --- add getdeps lib dirs as rpaths so
# delocate can resolve and bundle transitive deps (libfolly, etc.).
...
if [[ -f "$librebalancer" ]]; then
    ...
    if [[ -f "$prefix_file" ]]; then
        ...
        echo "repair_macos: added getdeps lib rpaths to librebalancer.dylib"
    else
