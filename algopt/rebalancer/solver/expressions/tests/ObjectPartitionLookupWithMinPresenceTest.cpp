 ...
 protected:
  ...
  makeObjectPartitionLookupWithMinPresence(
      ...
      folly::F14FastMap<
          ...
          folly::F14FastSet<entities::GroupId>> forcePresent = {}) const {
...
// forcePresent: a (group, scope item) listed in forcePresent has its presence
// floor honored even when the group's actual utilization in the scope item
// drops to 0 -- as if a frozen object kept step(util) == 1. Groups not listed
// keep the default step(util) gating.
CO_TEST_F(
    ObjectPartitionLookupWithMinPresenceTest,
    ForcePresentHonorsFloorAtZeroUtil) {
  const auto universe = co_await setUpUniverse();
  auto assignment = getInitialAssignment(*universe);

  // forcePresent only for group2 in region1. The floor value comes from
  // groupToPresenceWeight (2.0 for group2).
  auto withForcePresent = makeObjectPartitionLookupWithMinPresence(
      universe,
      /*aggregationScopeItemId=*/region(1),
      /*groupIds=*/{group(1), group(2)},
      /*multiplierList=*/{},
      /*makeContinuousPenaltyTerm=*/false,
      /*roundUpGroupUtilOnScopeItem=*/false,
      /*forcePresent=*/{{region(1), {group(2)}}});
  auto withoutForcePresent = makeObjectPartitionLookupWithMinPresence(
      universe,
      /*aggregationScopeItemId=*/region(1),
      /*groupIds=*/{group(1), group(2)},
      /*multiplierList=*/{},
  ...
  // forcePresent). group1 util = max(3.18, 3.0) = 3.18; group2 = max(1.0, 2.0)
  // = 2.0; total = 5.18.
  EXPECT_NEAR(5.18, apply(withForcePresent, assignment, lpAssertOptions), 1e-8);
  EXPECT_NEAR(
      5.18, apply(withoutForcePresent, assignment, lpAssertOptions), 1e-8);
  ...
  // Without forcePresent: step(0) == 0 -> group2 floor not applied -> group2 =
  // 0. total = group1 (3.18) + group2 (0) = 3.18.
  EXPECT_NEAR(
      3.18,
      evaluate(withoutForcePresent, changes, assignment, lpAssertOptions),
      1e-8);

  // With forcePresent: floor honored even at zero util -> group2 = max(2.0, 0)
  // = 2.0. total = group1 (3.18) + group2 (2.0) = 5.18.
  EXPECT_NEAR(
      5.18,
      evaluate(withForcePresent, changes, assignment, lpAssertOptions),
      1e-8);
}

// forcePresent is per (group, scope item): a group NOT listed keeps step()
// gating and drops to 0 at zero util, while a listed group keeps its floor.
CO_TEST_F(
    ObjectPartitionLookupWithMinPresenceTest,
    ForcePresentIsSelectivePerGroup) {
  const auto universe = co_await setUpUniverse();
  auto assignment = getInitialAssignment(*universe);

  // forcePresent only for group2 (group1 is intentionally not listed).
  auto objectPartitionLookup = makeObjectPartitionLookupWithMinPresence(
      universe,
      /*aggregationScopeItemId=*/region(1),
      /*groupIds=*/{group(1), group(2)},
  ...
  // group1 is NOT force-present -> step(0) == 0 -> floor not applied -> 0.
  // group2 IS force-present -> floor honored -> max(2.0, 0) = 2.0.
  // total = 0 + 2.0 = 2.0.
  EXPECT_NEAR(
      2.0,
...
// forcePresent with roundUp + multipliers: the forced floor flows through the
// same rounding/multiplier pipeline as a normally-present group.
CO_TEST_F(
    ObjectPartitionLookupWithMinPresenceTest,
    ForcePresentWithRoundUpAndMultiplier) {
  const auto universe = co_await setUpUniverse();
  auto assignment = getInitialAssignment(*universe);
