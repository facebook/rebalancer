...
folly::coro::Task<std::string> proxyCall(
  ...
  // forwarded as an RPC argument, never used to choose a destination.
  const folly::SocketAddress addr(
      config->backendHost, config->backendPort, /*allowNameLookup=*/true);
  auto socket = folly::AsyncSocket::newSocket(
      backendEvb, addr, static_cast<uint32_t>(config->backendTimeout.count()));
  auto channel =
