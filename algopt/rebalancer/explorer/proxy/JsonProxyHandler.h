...
struct ProxyConfig {
  // The single, configured Explorer backend the proxy always dials. The
  // caller-supplied `Handle` is never used to pick a destination (SSRF-safe);
  // it is only forwarded as an RPC argument.
